...
-type(amqp_method_field_name() ::
       ( frame_max | exclusive | out_of_band | requeue
       | channel_id | active | nowait | prefetch_size
       | exchange | active | global | reply_code
       | reply_text | class_id | reply_code | method_id
       | ticket | queue | consumer_tag | realm
       | exclusive | version_major | delivery_tag | passive
       | nowait | active | write | read
       | version_minor | ticket | reply_text | mechanisms
       | redelivered | locales | ticket | exchange
       | mechanism | response | server_properties | locale
       | exchange | auto_delete | challenge | nowait
       | response | routing_key | routing_key | channel_max
       | ticket | heartbeat | if_unused | channel_max
       | frame_max | heartbeat | virtual_host | ticket
       | destination | capabilities | insist | consumer_tag
       | nowait | prefetch_count | source | arguments
       | exchange | reply_code | reply_text | destination
       | consumer_tag | source | client_properties | routing_key
       | routing_key | nowait | arguments | ticket
       | exchange | cluster_id | delivery_tag | redelivered
       | type | exchange | routing_key | message_count
       | ticket | queue | passive | passive
       | immediate | known_hosts | consumer_tag | durable
       | exclusive | requeue | auto_delete | nowait
       | arguments | durable | nowait | queue
       | message_count | consumer_count | requeue | delivery_tag
       | consumer_tag | routing_key | ticket | queue
       | exchange | routing_key | nowait | multiple
       | ticket | arguments | internal | delivery_tag
       | multiple | requeue | ticket | delivery_tag
       | ticket | queue | class_id | nowait
       | arguments | message_count | method_id | arguments
       | ticket | queue | if_unused | if_empty
       | nowait | no_local | queue | message_count
       | nowait | ticket | no_ack | no_ack
       | queue | mandatory | exchange | routing_key
       | arguments )).
