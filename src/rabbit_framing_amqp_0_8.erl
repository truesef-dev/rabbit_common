...
-type(amqp_method_field_name() ::
       ( delivery_tag | class_id | method_id | prefetch_size
       | exchange | routing_key | multiple | exchange
       | version_major | realm | server_properties | frame_max
       | ticket | mechanisms | method_id | locales
       | write | requeue | read | mechanism
       | exchange | virtual_host | response | exchange
       | no_ack | locale | reply_code | reply_text
       | no_local | challenge | exchange | response
       | redelivered | passive | queue | redelivered
       | durable | consumer_tag | string_1 | auto_delete
       | internal | nowait | arguments | routing_key
       | frame_max | mandatory | ticket | channel_max
       | routing_key | ticket | exchange | consumer_tag
       | if_unused | insist | message_count | string_2
       | version_minor | known_hosts | out_of_band | dtx_identifier
       | host | integer_4 | queue | ticket
       | passive | reply_code | exclusive | requeue
       | reply_text | auto_delete | reply_text | nowait
       | identifier | arguments | immediate | requeue
       | queue | delivery_tag | passive | message_count
       | consumer_count | no_local | exchange | active
       | staged_size | ticket | queue | active
       | exclusive | exchange | prefetch_size | routing_key
       | exchange | nowait | details | result
       | queue | consumer_tag | reply_code | nowait
       | capabilities | reply_text | reply_text | requeue
       | integer_3 | class_id | ticket | delivery_tag
       | queue | heartbeat | nowait | client_properties
       | active | ticket | nowait | consume_rate
       | message_count | mandatory | delivery_tag | immediate
       | consumer_tag | heartbeat | ticket | ticket
       | content_size | queue | mandatory | if_unused
       | meta_data | if_empty | nowait | prefetch_count
       | consumer_tag | global | ticket | reply_code
       | message_count | table | exchange | consumer_tag
       | delivery_tag | ticket | integer_1 | operation
       | queue | routing_key | exchange | consumer_tag
       | routing_key | routing_key | reply_text | operation
       | arguments | no_ack | exclusive | nowait
       | delivery_tag | consumer_tag | arguments | ticket
       | cluster_id | routing_key | consumer_tag | consumer_tag
       | result | nowait | integer_2 | prefetch_size
       | prefetch_count | global | delivery_tag | known_hosts
       | ticket | multiple | string_result | string_op
       | exchange | consumer_tag | ticket | global
       | queue | redelivered | nowait | consumer_tag
       | prefetch_count | exchange | no_local | routing_key
       | no_ack | queue | identifier | exclusive
       | content_checksum | integer_result | nowait | durable
       | type | consumer_tag | integer_op | delivery_tag
       | ticket | exchange | consumer_tag | routing_key
       | nowait | routing_key | reply_code | reply_code
       | immediate | exclusive | identifier | consumer_tag
       | queue | channel_max )).
